...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (options.length < 2) {
      return interaction.reply({ embeds: [embeds.error('Invalid Options', 'Please provide at least 2 options separated by commas.')] });
    }
    const choice = options[Math.floor(Math.random() * options.length)];
    await interaction.reply({ embeds: [embeds.success('Result', `I choose: **${choice}**`)] });
  },
};

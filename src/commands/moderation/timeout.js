...
module.exports = {
  ...
  async execute(interaction) {
    ...
    logger.info(`${interaction.user.tag} timed out ${target.user.tag} for ${duration}m: ${reason}`);
    await interaction.reply({ embeds: [embeds.success('Timed Out', `${target.user.tag} has been timed out for ${duration} minute(s).\nReason: ${reason}`)] });
  },
};
